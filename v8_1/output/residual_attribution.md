...
注：r train MAE(1715)>ext_err train(1578) 因 base_A_oof 为早停 walk-forward OOF（训练折上退化）；val 为干净 eval-only，v6 修正 215 MW 可信。
...
**combined 期内 R²=-0.061**（负=特征噪声, 期内即不可学）。ext_err 即便用全 126 特征，**期内 holdout R²<0** -> 外部原始误差对当前特征**期内即不可预测**（比 Phase0 的 r −0.136 更早确认天花板：误差从一开始就是特征噪声，非 v6 学不动）。
**关键区分**：v6 仍在 val 修正 215 MW（ext_err 1660->r 1446），但此修正来自 **bias/结构校正**（OOF bias、96dim slot bias、分段 bias、MOS anchor），**非预测 ext_err**（探针 R²≈0 证不可预测）。即 v6 修的是"系统偏差"不是"误差值"。
combined 跨年 transfer R²=-0.004（≈0，无跨年可迁移的 ext_err 预测信号）。
...
|---|---|---|---|
| Demand shift | 8 | 1566 | 0.94 |
| Input anomaly | 8 | 2338 | 1.41 |
| Weather OOD | 16 | 1376 | 0.83 |
| Likely renewable | 8 | 1814 | 1.09 |
| Unknown | 67 | 1640 | 0.99 |
...
├── 可学习 = v6 bias/结构校正 (val 修正 215 MW: ext_err 1660->r 1446)
│   ↑ 非 ext_err 预测 (探针期内 R²=-0.061≈0, feature noise); 来源=OOF/slot/分段 bias+MOS
...
└── 不可学习 (天花板 ≈ r, ext_err 期内 R²≈0 = 特征噪声)
    ├── Weather OOD (novelty)       corr|r|=+0.156 (ratio<1 翻, 不稳)
    ├── Input anomaly (pred_load)   corr|r|=+0.157 (稳, GO flag, 标签 ratio 1.41)
    ├── Renewable (proxy)           符号命中=0.495 (排除: 气象隐含可再生不可迁移)
    └── Random/Unknown              floor (符号通道缺失, 主体)
```


## F. 与既有诊断统一

- **FDS**：ext_error 83.7% unlearnable。本处 ext_err combined 期内 R²=-0.061（≈0/负，期内即不可学，同 FDS 结论）。
- **Phase0**：r 期内 R²=−0.136（残差=特征噪声）。本处 ext_err 期内 R²=-0.061（亦≈0/负）--**外部原始误差从一开始就不可学**，v6 的 215 MW 来自 bias 校正非误差预测。两者一致且本处更早确认天花板。
