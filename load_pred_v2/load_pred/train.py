...
def run_train(verbose: bool = True):
    ...
    if verbose:
        print("[3.5/6] 计算 OOF 每小时偏置校正 + 漂移方向校正 ...")
    model.hour_bias, model.drift_corr = compute_hour_bias(times, X, pred_load, actual, usable, cfg, best_it)
