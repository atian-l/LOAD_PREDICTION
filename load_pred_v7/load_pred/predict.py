...
用法:
    ...
    python -m load_pred.predict                 # 默认=今天
"""
...
def predict_d1(run_date, run_hour=C.DEFAULT_RUN_HOUR, run_minute=C.DEFAULT_RUN_MINUTE,
    ...
    # ---- 仅使用运行时可获得的数据 ----
    # 1) 外部预测负荷（全历史 + D+1；用于当前值与滞后/滚动）
    pred_load = dl.pred_load_series()

    # 2) 气象：仅起报时间 <= 运行时刻，再按“最晚起报”去重
    weather = dl.load_weather_dedup(run_time=run_dt)

    # 3) 特征（与训练完全一致的构造逻辑）
    #    bug#1 修复：训练在 full_time_index（全历史）上构造特征；若预测仅在 96 点 D+1 上构造，
    #    则窗口>96 的 rolling（pl_norm/irrad_anom_672/pl_wr_roll_672…）与 shift(>=96)
    #    （pl_wr_diff_96/672、pred_load_lag_*）会全 NaN（train/serve skew），LightGBM 路由到
    #    默认分支=丢失特征，使上报 val MAE 相对真实部署偏乐观。故在足够长的历史回看窗口上
    #    构造特征、再取 D+1 96 点。最长滞后/窗口=672 步(7 天)，取 14 天回看(>672+96)保证 D+1
    #    各点特征与训练 bit-identical（rolling/shift 仅回看 ≤672，14 天窗口已完整覆盖）。
    ...
    # 应用错配/残差模型（训练期拟合，预测期复用），加入 pl_weather_residual 等需拟合特征
...
def _save_outputs(out: pd.DataFrame, run_date):
    ...
    # 按运行日期命名（Constraint：prediction_YYYYMMDD.csv，YYYYMMDD=运行日期）
...
def main():
    ap = argparse.ArgumentParser(description="日前(D+1)负荷预测 - 预测模式")
