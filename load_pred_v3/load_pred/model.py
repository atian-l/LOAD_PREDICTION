...
class EnsembleModel:
    ...
    def __init__(self, feature_cols: list[str], shrinkage: float = 1.0,
        ...
        # 每小时偏置校正（来自训练期 3-fold OOF，仅用训练数据估计；无泄露）。
        # predict 时减去 hour_bias[hour]，消除模型系统性的小时偏置（实验确认 -6 MW）。
    ...
    def predict_load(self, X: pd.DataFrame, pred_load_at_T: pd.Series) -> np.ndarray:
        ...
        # 每小时偏置校正（仅用训练期 OOF 估计；predict 时按小时索引减去）
        if self.hour_bias is not None:
            pred = pred - self.hour_bias[hours]
        ...
        # 阈值场景校正（OOF 估计的 shift；晴午间高估/阴雨天低估；无泄露）
        for feat_name, thr, hours_list, shift in self.threshold_corr:
            fv = X[feat_name].values.astype(float)
            sel = fv > thr
            if hours_list is not None:
                sel = sel & np.isin(hours, hours_list)
            if shift != 0.0:
    ...
    def save(self, path: Path) -> None:
        ...
        bundle = {
            ...
            "threshold_corr": [(n, t, h, s) for (n, t, h, s) in self.threshold_corr],
        }
    ...
    def load(cls, path: Path) -> "EnsembleModel":
        ...
        obj = cls(feature_cols=bundle["feature_cols"],
                  ...
                  threshold_corr=bundle.get("threshold_corr"))
